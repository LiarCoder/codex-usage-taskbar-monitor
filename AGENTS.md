...
`build.rs` embeds Windows resources. Release automation lives in `.github/workflows/release.yml`.
...
- Keep commits focused and use the repository's Conventional Commit style; use `chore(release): bump version to X.Y.Z` for version bumps
- PR titles and descriptions should be in Chinese and include user-visible changes and validation performed

Never commit Codex credentials, tokens, logs, local settings, or generated release artifacts. Preserve CodexRadar's opt-in and privacy behavior. The user-level `$release-app-version` skill is available only through explicit user invocation.
